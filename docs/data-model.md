...
すべてLv50固定・個体値31固定なので、実数値計算は単純化できる:

```
HP   = floor((base*2 + 31 + floor(sp_hp /4)) * 50 / 100) + 50 + 10
other = floor( (floor((base*2 + 31 + floor(sp_x /4)) * 50 / 100) + 5) * nature_modifier )
```

- 従来のEVは `4 EV → 1 SP`、上乗せ分は `8 EV → +1 SP` で換算される（HOME転送時）
- SPの合計上限、各ステの上限はゲーム内仕様に従う（データパックで持つ）
- 性格補正は従来通り（+10% / -10%）、HOME出張時のデフォルト性格は変更可能（VPでトレーニング）

**重要**: ダメージ計算パッケージはこの式に合わせて実装する。Lv50固定・IV31固定の前提でロジックが大幅に短くなる。
